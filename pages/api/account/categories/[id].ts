...
const deleteCategory = async (req:NextApiRequest, res:NextApiResponse):Promise<void> => {
  try {
    ...
    const category:ICategory = await Category.findByIdAndDelete(req.query.id)
...
const editCategory = async (req:NextApiRequest, res:NextApiResponse):Promise<void> => {
  try {
    const result = auth(req, res) as IDecode
    const user:IUser = await User.findById(result.userId)
